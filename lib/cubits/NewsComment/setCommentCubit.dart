...
class SetCommentCubit extends Cubit<SetCommentState> {
  ...
  void setComment({required String userId, required String parentId, required String newsId, required String message}) async {
    emit(SetCommentFetchInProgress());
    try {
      final result = await _setCommentRepository.setComment(userId: userId, message: message, newsId: newsId, parentId: parentId);
